package com.arango.archunit.spring;
...
public class APIControllerTest {
        ...
        private final ArchRule loggers_should_be_private_static_final = methods().that().areDeclaredInClassesThat()
                        .areAnnotatedWith(RestController.class).and().arePublic().should()
                        .haveRawReturnType(ResponseEntity.class)
                        .because("we don't want to expose domain models directly");

        @ArchTest
        private final ArchRule rule = classes().that().areAnnotatedWith(RestController.class).should()
